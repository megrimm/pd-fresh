...
for pd_darwin in `find . -name '*.pd_darwin'`; do
    LIBS=`otool -L $pd_darwin | sed -n 's|.*/usr/local/lib/\(.*\.dylib\).*|\1|p'`
    if [ "x$LIBS" != "x" ]; then
	...
	for lib in $LIBS; do
	    ...
	    install_name_tool -change /usr/local/lib/$lib @loader_path/$new_lib $pd_darwin
	done
	echo " "
    fi
done
...
for dylib in $PD_APP_LIB/*.dylib; do
	...
	for lib in $LIBS; do
	    ...
	    install_name_tool -id @loader_path/$new_lib $PD_APP_LIB/$new_lib
	    install_name_tool -change /usr/local/lib/$lib @loader_path/$new_lib $dylib
	done
	echo " "
    fi
done
